...
export const postCriarAdminAction =
	(token, email, nome, documento, celular) => async (dispatch) => {
		try {
			const res = await postCriarAdmin(
				token,
				email,
				nome,
				documento,
...
export const getListaBannerAction =
	(token, page, like, order, mostrar, tipo) => async (dispatch) => {
		try {
...
export const getContaEmpresaAction = (token, id) => async (dispatch) => {
	try {
		const res = await getContaEmpresa(token, id);
		dispatch({
			type: GET_CONTA_EMPRESA,
			payload: res.data,
		});
		return false;
	} catch (err) {
...
export const postNotificacaoAction =
	(token, titulo = '', mensagem = '', contas = '') =>
	async (dispatch) => {
		try {
			const res = await postNotificacao(token, titulo, mensagem, contas);
			dispatch({
