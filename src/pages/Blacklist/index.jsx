...
const columns = [
  {
    ...
    FullObject: (obj) => (
      <Typography>
        {documentMask(obj?.conta?.razao_social ?? obj?.conta?.nome)}
      </Typography>
    ),
  },
  {
    ...
    FullObject: (obj) => (
      <Typography>
        {documentMask(obj?.conta?.cnpj ?? obj?.conta?.documento)}
      </Typography>
    ),
  },
  {
...
export default function Blacklist() {
  ...
  const filters = `documento=${filter.documento}&mostrar=${filter.mostrar}`;

  const getData = async (token, page = 1) => {
    setLoading(true);
    try {
      const { data } = await getBlacklist(token, page, filters);
      setListaContas(data);
    } catch (err) {
      console.log(err);
    } finally {
  ...
  const handleRemoveFromBlacklist = async (conta_id) => {
    setLoading(true);
    try {
  ...
  return (
    <Box className={useStyles.root}>
      <Box className={useStyles.headerContainer}>
        ...
        >
          <Typography className={useStyles.pageTitle}>Blacklist</Typography>
                ...
                <CustomTable
                  ...
                  Editar={({ row }) => (
                    <MenuOptionsTable
                      ...
                      getData={getData}
                      deleteCallback={() =>
                        handleRemoveFromBlacklist(row?.conta_id)
                      }
                    />
                  )}
                />
