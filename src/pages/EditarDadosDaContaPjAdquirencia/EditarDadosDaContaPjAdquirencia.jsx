...
import { postUserRepresentante, putEditartUser } from "../../services/services";
...
export default function EditarDadosDaContaPj() {
  ...
  const [
    openModalCriarContaRepresentante,
    setOpenModalCriarContaRepresentante,
  ] = useState(false);
  const Editar = (row) => {
    const [anchorEl, setAnchorEl] = useState(null);
    ...
    return (
      <Box>
        ...
        <ModalEditar
          show={openModalEditar}
          setShow={setOpenModalEditar}
          getData={() => dispatch(loadContaId(token, id, true, false, true))}
        />
      </Box>
    );
  };
          ...
          >
            ...
            >
              ...
              <TabPanel value={value} index={2} dir={theme.direction}>
                ...
                >
                  <Box>
                    ...
                    >
                      ...
                      <CustomButton
                        onClick={() =>
                          setOpenModalCriarContaRepresentante(true)
                        }
                        color="purple"
                        style={{ width: "250px", marginTop: "24px" }}
                      >
                        <Typography>Cadastrar representante</Typography>
                      </CustomButton>
                    </Box>
                ...
                <ModalCriarContaRepresentante
                  show={openModalCriarContaRepresentante}
                  setShow={setOpenModalCriarContaRepresentante}
                  getData={() =>
                    dispatch(loadContaId(token, id, true, false, true))
                  }
                />
...
function ModalEditar({ show, setShow, getData }) {
  const token = useAuth();
  const classes = useStyles();
  const [loading, setLoading] = useState(false);
  ...
  const handleEditarRepresentante = async () => {
    setLoading(true);
    try {
      await putEditartUser(token, show?.id, dadosRepresentanteEditar);
      toast.success("Representante editado com sucesso");
      getData();
      handleClose();
    } catch (err) {
      console.log(err);
      toast.error("Erro ao editar representante. Tente novamente.");
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    if (!!show) {
      setDadosRepresentanteEditar({
        nome: show?.nome,
        permissao: show?.permissao_master,
      });
    }
  }, [show]);
      ...
      <Box className={classes.modal}>
        ...
        >
          ...
          >
            ...
            <CustomCloseButton color="purple" onClick={handleClose} />
          </Box>
          ...
          <Box style={{ marginTop: "20px" }}>
            ...
            >
              {show?.nome} tem amplos poderes para fazer operações bancárias em
              nome da secretaria?
            </Typography>
            ...
            <Box style={{ display: "flex", marginTop: "10px" }}>
              <Checkbox
                color="primary"
                checked={dadosRepresentanteEditar?.permissao}
                onChange={() => {
                  setDadosRepresentanteEditar({
            ...
            >
              <CustomButton
                ...
                loading={loading}
              >
                <Typography>Editar representante</Typography>
              </CustomButton>
            </Box>
          </Box>
        </Box>
...
function ModalCriarContaRepresentante({ show, setShow, getData }) {
  const token = useAuth();
  const classes = useStyles();
  const id = useParams()?.id ?? "";
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    console.log(dadosRepresentante);
    setLoading(true);
    setErrorsRepresentante({});

    try {
      await postUserRepresentante(token, dadosRepresentante);
      ...
      setErrorsRepresentante(err?.response?.data?.errors);
    }
    setLoading(false);
  };
