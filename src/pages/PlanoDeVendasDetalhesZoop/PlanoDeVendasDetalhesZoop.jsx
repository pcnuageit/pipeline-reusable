...
import AddAplicationSalesPlanSubscriptionModal from '../../components/AddAplicationSalesPlanSubscriptionModal/AddAplicationSalesPlanSubscriptionModal';
	...
	const columns = [
		...
		{ headerText: 'Email', key: 'conta.email' },
		{
			...
			Teste: (item) => {
				return (
					<Typography>
						{item.conta.cnpj ?? item.conta.documento}
					</Typography>
				);
			},
		},
								...
								<Box display="flex" flexWrap="wrap" marginTop="12px">
									<Box flexGrow={1}>
										...
										<Box marginTop="12px">
											...
											>
												R${' '}
												{(salesPlan.maximum_amount / 100).toFixed(
													2
												) ?? '-'}
											</Typography>
										</Box>
